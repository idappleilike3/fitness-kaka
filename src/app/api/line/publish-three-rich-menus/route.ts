...
const definitions = [
  {
    ...
    actions: [`${BASE}/member-login`, `${BASE}/#how`, `${BASE}/#features`, `${BASE}/#plans`, "https://lin.ee/5rxQDpa", `${BASE}/faq`],
  },
  {
    key: "trial",
    name: "健身卡卡｜7 天體驗版",
    image: `${BASE}/images/line-imge/%E5%9C%96%E6%96%877%E5%A4%A9%E9%AB%94%E9%A9%97.png`,
    actions: [`${LIFF}?view=photo`, `${LIFF}?view=today`, `${LIFF}?view=challenge`, `${BASE}/#how`, `${LIFF}?view=trial`, `${BASE}/#plans`],
  },
  {
    key: "member",
    name: "健身卡卡｜正式會員版",
    image: `${BASE}/images/line-imge/kaka-rich-menu-paid.jpg`,
    actions: [`${LIFF}?view=photo`, `${LIFF}?view=today`, LIFF, `${LIFF}?view=challenge`, `${LIFF}?view=records`, `${BASE}/#plans`],
  },
] as const;
...
function menuBody(name: string, actions: readonly string[]) {
  ...
  const xs = [0, 834, 1667];
  return {
    size: { width: 2500, height: 1686 }, selected: true, name, chatBarText: "卡卡教練選單",
    areas: actions.map((uri, index) => ({
      bounds: { x: xs[index % 3], y: index < 3 ? 0 : 843, width: widths[index % 3], height: 843 },
      action: { type: "uri", uri },
    })),
  };
}

async function lineRequest(path: string, init: RequestInit, dataApi = false) {
  const { LINE_CHANNEL_ACCESS_TOKEN } = getLineEnv();
  const response = await fetch(`${dataApi ? LINE_DATA_API : LINE_API}${path}`, {
    ...init,
    headers: { Authorization: `Bearer ${LINE_CHANNEL_ACCESS_TOKEN}`, ...(init.headers || {}) },
  });
...
async function publish(definition: (typeof definitions)[number]) {
  const create = await lineRequest("/richmenu", {
    method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(menuBody(definition.name, definition.actions)),
  });
  ...
  const image = await sharp(Buffer.from(await source.arrayBuffer())).resize(2500, 1686, { fit: "fill" }).jpeg({ quality: 82, mozjpeg: true, chromaSubsampling: "4:2:0" }).toBuffer();
  await lineRequest(`/richmenu/${encodeURIComponent(richMenuId)}/content`, {
    method: "POST", headers: { "Content-Type": "image/jpeg" }, body: new Uint8Array(image),
  }, true);
