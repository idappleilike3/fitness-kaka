...
export function mealResultMessage(params: {
  ...
  return [
    ...
    `脂肪：${params.fatG}g`,
    "",
    `今日已確認熱量：${params.todayKcal.toLocaleString("zh-TW")} kcal`,
    `確認後預計累計：${params.projectedKcal.toLocaleString("zh-TW")} kcal、蛋白質 ${params.projectedProteinG}g`,
    `確認後預計還能吃：${params.remainingKcal.toLocaleString("zh-TW")} kcal`,
    `確認後蛋白質還差：${params.proteinLeft}g`,
    "",
