...
- 補回 v10 歷史素材 `story-roadmap.webp` 的圖片路由映射，修正正式站 404。
...
# 2026-08-04 粉紫霓虹互動版補充

- 首頁加入深紫透視格紋、游標視差與最大 6 度的卡片 3D 浮動；觸控裝置與減少動態效果設定會自動停用。
- 外食版、超商版、居家版改為可切換情境，分別顯示既有圖片、餐點、熱量、蛋白質與份量指引。
- 四個核心功能故事補上既有餐點／卡卡圖片，不再只使用文字與圖示。
- 新增免費卡卡規則問答：預設問題、自由輸入關鍵字比對、延伸問題、醫療安全分流、LINE 真人引導。
- 問答不呼叫 OpenAI 或其他付費 API；音效使用瀏覽器 Web Audio，預設靜音並可由使用者切換。
- 聚焦首頁測試 41/41 通過；Next.js 正式建置通過。
- 完整回歸 151/154 通過。既有 3 項失敗位於 `tests/line/messages.test.ts`（2）與 `tests/services/meal-flow.test.ts`（1），與首頁修改無關。
